...
type AppState = "setup" | "input" | "thinking" | "review" | "executing" | "done";
...
export default function App() {
  ...
  const handleInput = useCallback(async (value: string) => {
    if (!isGitRepo()) {
      setError("Not inside a git repository. Please navigate to a git repo and try again.");
      return;
    }
    setRequest(value);
    setError("");
  ...
  const handleExecutionComplete = useCallback((executionResults: CommandResult[]) => {
  ...
  return (
    <Box flexDirection="column">
      ...
      {state === "done" && <Done results={results} onContinue={handleContinue} />}
    </Box>
  );
}
...
function Done({
  ...
  return (
    <Box flexDirection="column" padding={1}>
      <Text bold color={allSuccess ? "green" : "red"}>
        {allSuccess ? "All commands completed successfully!" : "Some commands failed."}
      </Text>
